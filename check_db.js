...
if (!supabaseUrl || !supabaseKey) {
  console.error(
    "Missing SUPABASE URL or SERVICE ROLE KEY in env. Env keys:",
...
async function checkSchema() {
  ...
  if (error) {
    ...
    console.log(
      "Success reading profiles. Sample data keys:",
      data && data.length > 0 ? Object.keys(data[0]) : "Empty",
    )
  }
  ...
  for (const col of testColumns) {
    const { data: testData, error: testErr } = await supabase.from("profiles").select(col).limit(1)
    if (testErr) {
