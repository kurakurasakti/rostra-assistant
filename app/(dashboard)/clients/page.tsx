...
import { Loader2, Plus, Search, Upload, UserPlus, Users } from "lucide-react"
...
export default function ClientsPage() {
  ...
  async function loadClients() {
    ...
    setClients(
      (clientsRes.data ?? []).map((c) => ({ ...c, active_order_count: countMap[c.id] ?? 0 })),
    )
    setLoading(false)
  }
          ...
          >
            ...
            <SheetContent className="w-full sm:max-w-md flex flex-col p-0">
              <SheetHeader className="px-6 pt-6 pb-4 border-b border-border shrink-0">
                <SheetTitle className="font-display font-semibold text-lg">
                  Tambah Client Baru
                </SheetTitle>
