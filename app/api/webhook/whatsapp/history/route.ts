...
export async function POST(request: Request) {
  ...
  console.log("[webhook/history] received batch:", {
...
async function processHistoryBatch(payload: any) {
  ...
  const senders = items.map((m) => normalizeWANumber(m.sender ?? "")).filter(Boolean) as string[]
  ...
  for (const item of items) {
    ...
    console.log(
      `[webhook/history] item: sender=${sender} normalized=${normalizedSender ?? "SKIPPED"} name=${item.name || "NONE"}`,
    )
    if (!normalizedSender) continue

    // Security scan for injection
    const injectionCheck = scanForInjection(message)
    const classification = injectionCheck.isSuspicious ? "injection_attempt" : "tidak_diketahui"
  ...
  console.log(
    `[webhook/history] done: ${rows.length}/${items.length} rows processed for user ${userId.slice(0, 8)}...`,
  )
}
