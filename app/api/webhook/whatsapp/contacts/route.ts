...
export async function POST(request: Request) {
  ...
  processContacts(body).catch((err) =>
    console.error('[webhook/contacts] processing error:', err)
  )
...
async function processContacts(payload: any) {
  ...
  if (existingClientsToUpdate.length > 0) {
    console.log(`[webhook/contacts] updating ${existingClientsToUpdate.length} existing client names`)
  ...
  console.log(`[webhook/contacts] updating sender_name and client_id in inbox_messages for ${uniqueNumbers.length} numbers`)
