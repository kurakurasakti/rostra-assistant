...
test.describe("F6 — Client Management", () => {
  test("F6.1 — Add client manually", async ({ page }) => {
    ...
    await page.fill('input[placeholder="08123456789 atau 628123456789"]', "081234567890")
  ...
  test("F6.3 — Client detail — AI notes", async ({ page }) => {
    ...
    await page.fill(
      "textarea",
      "Pelanggan VIP, sudah order 5x. Boleh diskon max 10%. Panggil Kak Siti.",
    )
    ...
    await page.reload()
    await expect(page.locator("textarea")).toHaveValue(/Pelanggan VIP/)
  })
})
