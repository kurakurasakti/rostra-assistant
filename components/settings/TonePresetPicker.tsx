...
import { Briefcase, Coffee, Sliders, Smile } from "lucide-react"
...
export const TONE_PRESETS = [
  ...
  },
  {
    ...
    icon: Briefcase,
    template:
      "Profesional dan sopan. Gunakan bahasa baku, hindari singkatan dan emoji. Jawaban to the point dan informatif.",
  },
  {
    id: "santai",
    label: "Santai",
    ...
    icon: Coffee,
    template:
      "Santai dan akrab, seperti mengobrol dengan teman dekat. Boleh pakai bahasa gaul ringan dan emoji. Tetap jelas dan membantu.",
  },
] as const
...
const deriveFromValue = (v: string) => TONE_PRESETS.find((p) => p.template === v)?.id ?? "custom"

export default function TonePresetPicker({ value, onSelect, customSlot }: TonePresetPickerProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2 max-w-[640px]">
        ...
        >
          <Sliders
            className={`w-4 h-4 mt-0.5 shrink-0 ${activeCardId === "custom" ? "text-primary" : "text-muted-foreground"}`}
          />
          <div className="space-y-0.5">
            <p className="text-xs font-semibold text-foreground">Kustom / AI Analisis</p>
            <p className="text-[10px] text-muted-foreground leading-snug">
              Edit manual gaya chat atau pakai hasil analisis
            </p>
          </div>
        </button>
      </div>

      {activeCardId === "custom" && <div className="animate-enter">{customSlot}</div>}
    </div>
  )
}
